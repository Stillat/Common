...
class MathEnginesTest extends PHPUnit_Framework_TestCase {

	protected $supportedDrivers = ['bc', 'native'];
	...
	const DRIVER_INTERFACE = '\Stillat\Common\Math\ExpressionEngines\ExpressionEngineInterface';

	public function testSettingDriverReturnsInstance()
	{
		foreach ($this->supportedDrivers as $driverName)
		{
			$manager = new MathManager($driverName);

			$this->assertInstanceOf(self::DRIVER_INTERFACE, $manager->getDriver());
	...
	{
		$this->manager = new MathManager('bc');
	...
	{
		$this->manager = new MathManager('native');
		$this->manager->setPrecision($this->testPrecision);
	...
	{
		$manager = new MathManager('native');
		$manager->divide('5', '0');
	}
